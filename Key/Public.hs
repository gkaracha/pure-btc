...
data UPublicKey = UPubKey Word256 Word256 -- (x,y), prefix 0x04, shown as (8 + 256 + 256 = 520 bits)
  deriving (Eq)

instance Hex UPublicKey where
  showHex (UPubKey x y) = "04" ++ showHex x ++ showHex y
  readHex str
    | Just ("04",x,y) <- splitInThree 2 64 64 str
    = UPubKey (readHex x) (readHex y)
...
instance Hex PublicKey where
  ...
  readHex str
    | Just ("04",x,y) <- splitInThree 2 64 64 str
    = UPublicKey $ UPubKey (readHex x) (readHex y)
...
pubKeyToByteString (UPublicKey upub)
  = case upub of
      UPubKey w1 w2 -> fromBytes $ 0x04 : toBytes w1 ++ toBytes w2
...
publicKeyFromPrivateKey pk = case pk of
  UPrivateKey key -> UPublicKey (publicKeyFromUPrivateKey key)
  CPrivateKey key -> CPublicKey (publicKeyFromCPrivateKey key)
  where
    publicKeyFromUPrivateKey :: UPrivateKey -> UPublicKey
    publicKeyFromUPrivateKey (UPriKey (hashECM -> (x,y))) = UPubKey x y

    publicKeyFromCPrivateKey :: CPrivateKey -> CPublicKey
    publicKeyFromCPrivateKey (CPriKey (hashECM -> (x,y)))
      | even y    = CPKEven x
      | otherwise = CPKOdd  x
...
compressPublicKey (UPubKey x y) | even y    = CPKEven x
                            | otherwise = CPKOdd  x

uncompressPublicKey :: CPublicKey -> UPublicKey
uncompressPublicKey (CPKEven x) = uncurry UPubKey $ uncompress 0x02 x
uncompressPublicKey (CPKOdd  x) = uncurry UPubKey $ uncompress 0x03 x
